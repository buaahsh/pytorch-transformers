...
class CMSQAExample(object):
    ...
    def __init__(self, cmsqa_id, context_sentence, endings, label, context):
...
class InputFeatures(object):
    def __init__(self, example_id, choices_features, label):
        ...
        self.choices_features = [
            ...
            for _, input_ids, input_mask, segment_ids, context_input_ids in choices_features
        ]
        self.label = label
    ...
    with open(input_file, 'r', encoding='utf-8') as reader:
        for line in reader:
            ...
            for choice_id, choice in enumerate(data["question"]["choices"]):
                ...
                contexts[choice["label"]] = data["question"]['question_concept'] + ' ' + choice_text
...
def convert_examples_to_features(examples, tokenizer, max_seq_length, is_training):
    ...
    for example_index, example in enumerate(examples):
        ...
        if example_index < 5:
            ...
            for choice_idx, (tokens, input_ids, input_mask, segment_ids, _) in enumerate(choices_features):
...
def load_and_cache_examples(args, _file, tokenizer, evaluate=False):
    ...
    context_all_input_ids = torch.tensor(select_field(train_features, 'context_input_ids'), dtype=torch.long)
    train_data = TensorDataset(all_input_ids, all_input_mask, all_segment_ids, all_label, context_all_input_ids)
